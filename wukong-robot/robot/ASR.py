...
from .sdk import TencentSpeech, AliSpeech, XunfeiSpeech, BaiduSpeech, FunASREngine, VolcengineSpeech, PaddleSpeech
...
class PaddleSpeechASR(AbstractASR):

    SLUG = "paddlespeech-asr"

    def __init__(self, url, model_dir, **args):
        super(self.__class__, self).__init__()
        self.client = PaddleSpeech.paddleSpeech(url, model_dir)

    @classmethod
    def get_config(cls):
        # Try to get baidu_yuyin config from config
        return config.get("paddlespeech", {})

    def transcribe(self, fp):
        # 识别本地文件
        res = self.client.asr(fp)
        if res["code"] == 0:
            logger.info(f"{self.SLUG} 语音识别到了：{res['result']}")
            return "".join(res["result"])
        else:
            logger.info(f"{self.SLUG} 语音识别出错了: {res['message']}")
            return ""
