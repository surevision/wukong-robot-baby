...
class LifeCycleHandler(object):
    ...
    def _beep_hi(self, onCompleted=None):
        start_sounds = config.get("/sounds/start_sounds", False)
        logger.info(start_sounds)
        logger.info(start_sounds.split(","))
        hi = random.choice(start_sounds.split(","))
        # Player.play(constants.getData("beep_hi.wav"), onCompleted)
        Player.play(constants.getData(hi), onCompleted)

    def _beep_lo(self):
        end_sounds = config.get("/sounds/end_sounds", False)
        logger.info(end_sounds)
        logger.info(end_sounds.split(","))
        lo = random.choice(end_sounds.split(","))
        # Player.play(constants.getData("beep_lo.wav"))
        Player.play(constants.getData(lo))
